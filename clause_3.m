...
for classifier_index=0:9
    result = A_new_test*classifiers(:,classifier_index);
    classified_indexes = find(result==ones(classifier_count,1));
    for classified_index=1:length(classified_indexes)
...
for histogram_row=1:num_images
    ones_counter = 0;
    for histogram_column=1:10
...
for unclassified_index=1:length(error_unclassified)
    ...
    for histogram_column=1:10
        ...
        if ones_counter == 2
            if sign(A_new_test(histogram_row,:)*couple_classifiers(:,indexes(1),indexes(2))) == 1
                indexes(2) = -1;
                ones_counter = ones_counter -1;
            else
                indexes(1) = indexes(2);
